...
from seq2seq1.seq2seq import Seq2seq
from seq2seq1.searcher import Greedy_Search
from seq2seq1.searcher import Beam_Search
...
def prepare_data(type="train"):
    SRC = Field(tokenize=tokenize_de, unk_token='<unk>', pad_token='<pad>', init_token='<sos>', eos_token='<eos>', lower=True)
    TGT = Field(tokenize=tokenize_en, unk_token='<unk>', pad_token='<pad>', init_token='<sos>', eos_token='<eos>', lower=True)
    ...
    enc_emb_dim, enc_hid_dim, enc_n_layer, enc_dropout, input_dim  = 300, 300, 2, 0.1, len(SRC.vocab)
    dec_emb_dim, dec_hid_dim, dec_n_layer, dec_dropout, output_dim = 300, 300, 2, 0.1, len(TGT.vocab)
