...
class Greedy_Search(nn.Module):
    ...
    def forward(self, src):
        ...
        all_scores = torch.LongTensor(sos_input).unsqueeze(1).cuda()
...
class Node(object):
    def __init__(self, hidden, cell, previous_node, decoder_input, log_prob, length):
        self.hidden = hidden  # 当前单词的hidden_state
        self.cell = cell      # 当前单词的cell_state
...
class Beam_Search(nn.Module):
    ...
    def forward(self, src):
        ...
        for batch_idx in range(batch_size):
            ...
            # _cell:    [n_layer, 1, hid_dim]
            _decoder_input = torch.LongTensor([sos_idx]).cuda()
            _hidden  = hidden[:, batch_idx, :].unsqueeze(1)
            _cell    = cell[  :, batch_idx, :].unsqueeze(1)

            root = Node(hidden=_hidden, cell=_cell, previous_node=None, decoder_input=_decoder_input, log_prob=torch.FloatTensor([1]), length=1)
            q = Queue()
            q.put(root)
            ...
            while not q.empty():
                ...
                for idx in range(count):
                    ...
                    # decoder_cell:   [n_layer, 1, hid_dim]
                    decoder_output, decoder_hidden, decoder_cell = self.decoder(_decoder_input.contiguous(), _hidden.contiguous(), _cell.contiguous())
                    ...
                    for i in range(beam_size):
                        ...
                        new_node = Node(hidden=decoder_hidden, cell=decoder_cell, previous_node=node, decoder_input=_decoder_input, log_prob=prob, length=_len)
