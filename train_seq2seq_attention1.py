...
from seq2seq_attention1.seq2seq import Seq2seq
from seq2seq_attention1.searcher import Greedy_Search
from seq2seq_attention1.searcher import Beam_Search
...
def prepare_data(type="train"):
    ...
    enc_emb_size, enc_hid_size, enc_n_layer, enc_dropout, input_size  = 300, 300, 1, 0.1, len(SRC.vocab)
    dec_emb_size, dec_hid_size, dec_n_layer, dec_dropout, output_size = 300, 300, 1, 0.1, len(TGT.vocab)
    ...
    SRC.vocab.vectors.data[src_unk_idx] = torch.zeros(enc_emb_size)
    SRC.vocab.vectors.data[src_pad_idx] = torch.zeros(enc_emb_size)
    ...
    TGT.vocab.vectors.data[tgt_unk_idx] = torch.zeros(dec_emb_size)
    TGT.vocab.vectors.data[tgt_pad_idx] = torch.zeros(dec_emb_size)
...
def eval(model, valid_iterator, loss_fct):
    ...
    for idx, batch in enumerate(valid_iterator):
        ...
        tgt, tgt_len = batch.trg
        with torch.no_grad():
            output = model(src, src_len, tgt, tgt_len, teacher_forcing_ratio=0.2)
...
def do_train():
    ...
    for epoch in range(num_epoch):
        ...
        model_dir = "./checkpoint/seq2seq_attention1"
...
def do_predict():
    ...
    for idx, batch in enumerate(test_iterator):
        ...
        with torch.no_grad():
            all_tokens, all_scores = model(src, src_len)
