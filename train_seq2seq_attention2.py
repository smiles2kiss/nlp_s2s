...
from seq2seq_attention2.seq2seq import Seq2seq
from seq2seq_attention2.searcher import Greedy_Search
from seq2seq_attention2.searcher import Beam_Search
...
def prepare_data(type="train"):
    ...
    enc_emb_size, enc_hid_size, enc_n_layer, enc_dropout, input_size  = 300, 300, 2, 0.1, len(SRC.vocab)
    dec_emb_size, dec_hid_size, dec_n_layer, dec_dropout, output_size = 300, 300, 2, 0.1, len(TGT.vocab)
...
def do_predict():
    ...
    for idx, batch in enumerate(test_iterator):
        ...
        gth_tokens = tgt.cpu().detach().numpy().tolist()
